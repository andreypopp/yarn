...
if (isPosix) {
  postInstallContents = ['#!/bin/bash', 'WAITING_FOR_PACKAGE_POST_INSTALL=' + process.env['npm_package_name'], 'if [ "$npm_package_name" == "$WAITING_FOR_PACKAGE_POST_INSTALL" ]; then', '  esy build', 'fi'].join(os.EOL);
} else {
