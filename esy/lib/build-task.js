 ...
 */
function getPathsDelimiter(envVarName, buildPlatform) {
  ...
  return envVarName === 'OCAMLPATH' && buildPlatform === 'cygwin' ? ';' : buildPlatform === 'cygwin' || buildPlatform === 'linux' || buildPlatform === 'darwin' ? ':' : ';';
}
...
function fromBuildSpec(rootBuild, config, params) {
  ...
  function createTask(scopes) {
    ...
    evalIntoEnv(env, [{
      ...
      }).concat('$PATH').join(getPathsDelimiter('PATH', config.buildPlatform)),
      exported: true
    }, {
      name: 'MAN_PATH',
      value: Array.from(scopes.allDependencies.values()).map(function (dep) {
        return config.getFinalInstallPath(dep.spec, 'man');
      }).concat('$MAN_PATH').join(getPathsDelimiter('MAN_PATH', config.buildPlatform)),
      exported: true
    }]);
