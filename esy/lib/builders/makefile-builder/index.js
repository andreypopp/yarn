 ...
 */
var buildConfig = exports.buildConfig = (_buildConfig || _load_buildConfig()).createConfig({
 ...
 */
function renderToMakefile(sandbox, outputPath) {
  ...
  function createBuildRule(build, rule) {
    ...
    if (rule.withBuildEnv) {
      command.push((_outdent || _load_outdent()).default`
          $(shell_env_for__${(0, (_util || _load_util()).normalizePackageName)(build.name)}) source $(ESY_EJECT__ROOT)/bin/runtime.sh
  ...
  function visitTask(task) {
    ...
    ruleSet.push({
      type: 'define',
      name: `shell_env_for__${(0, (_util || _load_util()).normalizePackageName)(task.spec.name)}`,
      value: [{
...
function renderBuildTaskCommand(task) {
  ...
  return task.command.map(function (c) {
