...
If while developing you need a clean slate (delete all artifacts) you can run:
```
rm -rf node_modules yarn.lock ~/Library/Caches/Yarn ~/.esy _build _install
```

- `yarn.lock` is a file generated by `yarn` specifying all dependencies with an exact version number, commit number and exact url to download the source.
- `node_modules` is a folder containing all the deps of the current project, not nested.
- `~/Library/Caches/Yarn` a folder generated by `yarn` used as a cache containing the source of all dependencies you've ever installed.
- `~/.esy` a folder generated by `esy` used as a cache containing the build artifacts of all the deps you've ever built.
- `_build` and `_install` are folder generated by `esy` containing local build artifacts and install artifacts.
